...
//not working
public class CombinationSumMemoV1 {
    ...
    static class Node {
        ...
        public Node(Node node) {
            ...
            for (List<Integer> integers : node.val) {
                this.val.add(new ArrayList<>(integers));
            }
        }
    ...
    public static Node recur(int[] nums, int target, int i, int sum, List<Integer> vals, List<List<Integer>> ret, Node[][] memo) {
        ...
        Node newRet = new Node(ret1);

        for (List<Integer> integers : ret2.val) {
            if (!integers.isEmpty()) {
                newRet.val.add(integers);
            }
        }
